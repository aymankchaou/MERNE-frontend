...
import { addArticle, deleteArticle, editArticle, fetchArticleById, fetchArticles, fetchArticlesPagSer } from "../services/ArticleService";
...
export const getArticlesPage = createAsyncThunk(
    "article/getArticlesPage",
    async (mesparams, thunkAPI) => {
        const { rejectWithValue } = thunkAPI;
        try {
            const res = await fetchArticlesPagSer(mesparams.currentPage, mesparams.itemsPerPage);
            return res.data;
        }
        catch (error) {
            return rejectWithValue(error.message);
...
export const articleSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder
            ...
            //get articlesPage
            .addCase(getArticlesPage.pending, (state, action) => {
                state.isLoading = true;
                state.error = null;
            })
            .addCase(getArticlesPage.fulfilled, (state, action) => {
                state.articles = [action.payload, ...state.articles];
                state.isLoading = false;
                state.error = null;
                state.articles = action.payload;
            })
            .addCase(getArticlesPage.rejected, (state, action) => {
                state.isLoading = false;
                state.error = action.payload;
                console.log("impossible de se connecter au serveur")
            })
